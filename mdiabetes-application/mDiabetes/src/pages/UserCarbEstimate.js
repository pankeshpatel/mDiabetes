...
export default function UserCarbEstimate({route, navigation}) {
    ...
    useEffect(() => {
        setFoodItem(
            foodLog.map((val,indx)=>{
                let a = {item:val.name,quantity:(val.quantity + " " + val.units),"cho-est":""}
                val = a
                return val
            })
        )
    ...
    const handleChoEst = (v,i)=>{
        setFoodItem(foodItem.map((val,index)=>{
            if(i==index){
                val["cho-est"]=v
                return val
            }
  ...
  return (
    <SafeAreaView style={styles.root}>
        <ScrollView keyboardShouldPersistTaps='handled' contentContainerStyle={{flexGrow: 1}}>
            <View>
                <Text  style={styles.heading}>Please enter your carbohydrate estimates in gms.</Text>
            </View>
                ...
                <View style={styles.flexDisplay}>
                    <View style={{width:100}}>
                        <Text>{val.quantity + " " + val.units+ " "+ "of" + " " + val.name } </Text>
                    </View>
                    <View>
                        <TextInput style={{borderWidth:1,height:28,backgroundColor:"lightgrey",color:"black" ,flex:0.91,width:60}} onChangeText={(value)=>{handleChoEst(value,index)}} ></TextInput>
                    </View>
                    <View>
                        <Text>gms</Text>
                    </View>
