...
export default function PatientManagement({}) {
  ...
  const getPatientData = async () => {
    const patientData = await (
      await getData('patient-list', {authToken, username})
    ).json();
  ...
  const onRemovePatient = async Id => {
    const newPatients = patients.filter(
      ([key, value]) => value['_doc'].ID !== Id,
    );
    console.log('NEW PATIENTS : ', newPatients);
    setPatients(newPatients);
    await (
      await getData('patient-delete?ID=' + Id, {authToken, username})
    ).json();
    hideModal();
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.root}>
          ...
          return (
            <>
              ...
              <View key={value['_doc'].ID} style={styles.patient}>
                <Text numberOfLines={1} style={styles.patientID}>
                  {' '}
                  {value['_doc'].ID}{' '}
                </Text>
                <View style={styles.patientRight}>
                  ...
                  <View style={styles.patientTimestampWrapper}>
                    <Text style={styles.patientTimestamp}>
                      Created{' '}
                      {Date.now() - value['_doc'].timestamp < 1000
                        ? 'just now'
                        : DateTime.local()
                            .minus(Date.now() - value['_doc'].timestamp)
                            .toRelative()}
                    </Text>
                  </View>
                </View>
      ...
      {!loading && patients.length === 0 && (
        <Text style={styles.loadingText}>No patients to display.</Text>
      )}
    </ScrollView>
  );
}
