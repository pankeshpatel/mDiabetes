...
export default function LogFood({route, navigation}) {
  ...
  const submit = async () => {
    ...
    body.append('top_view_path', {
      name: topViewBinary.fileName,
      type: topViewBinary.type,
      uri:
        Platform.OS === 'ios'
          ? topViewBinary['uri'].replace('file://', '')
          : topViewBinary['uri'],
    });
    body.append('side_view_path', {
      name: sideViewBinary.fileName,
      type: sideViewBinary.type,
      uri:
        Platform.OS === 'ios'
          ? sideViewBinary['uri'].replace('file://', '')
          : sideViewBinary['uri'],
    });
    ...
    // fetch('http://ec2-54-89-144-199.compute-1.amazonaws.com:8080/api', {
      ...
      .then(res => {
        ...
        fetch(
          `${VOLUME_ESTIMATION_SERVER}` +
            'download/' +
            `${JSON.stringify(res.imageWithBbox)}`,
        )
          .then(res => res)
          .then(res => {
  ...
  const takePictureSide = () =>
    navigation.navigate('FoodCamera', {direction: 'side'});

  const togglePredictedFood = value => () => {
    if (values['predicted-food'].includes(value)) {
      edit('predicted-food')(values['predicted-food'].filter(f => f !== value));
    } else {
      edit('predicted-food')([...values['predicted-food'], value]);
    }
  };
  ...
  const getSelectedFoodNames = () => {
    const manual = (values['Extra Food'] || '')
      .split(',')
      .map(s => s.trim())
      .filter(s => s && s.length);
  ...
  if (step === 'camera') {
            ...
            {sideView == null ? (
              ...
              <TouchableOpacity
                ...
                <Image
                  size={50}
                  source={{uri: sideView}}
                  style={{width: 200, height: 130, marginTop: 70}}
                />
              </TouchableOpacity>
            )}
          </View>
      ...
      <ScrollView style={styles.root}>
        <View style={styles.selectTop}>
          ...
          <View style={styles.selectRight}>
            {apiResponse &&
              (apiResponse.top || apiResponse.side || {items: []}).items.map(
                item => (
                  <Checkbox.Item
                    key={item.name}
                    mode="android"
                    ...
                    onPress={togglePredictedFood(item.name)}
                    status={
                      values['predicted-food'].includes(item.name)
                        ? 'checked'
                        : 'unchecked'
                    }
                  />
        ...
        <View style={{flex: 1, marginBottom: '25%'}}>
          <Subheading>
            Enter the missed food item here separated by comma if any
          </Subheading>
          <TextInput
            style={styles.input}
            autoCorrect
    ...
    return (
      <ScrollView style={styles.root}>
        <Subheading>
          If there are any food item based on count, enter here
        </Subheading>
        {getSelectedFoodNames().map(item => (
          <View key={item} style={{alignItems: 'center', flexDirection: 'row'}}>
            <Checkbox.Item
              ...
              status="checked"
              status={
                values['based-on-count'].find(i => i.name === item)
                  ? 'checked'
                  : 'unchecked'
              }
            />
